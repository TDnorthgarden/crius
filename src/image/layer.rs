...
impl LayerStore {
    ...
    /// 添加层
    pub fn add_layer(&mut self, layer_id: String, parent_id: Option<String>, size: u64, media_type: &str) -> Result<()> {
        let now = Utc::now();
        ...
        if let Some(layer) = self.layers.get_mut(&layer_id) {
            ...
            debug!("Layer {} already exists, ref_count: {}", layer_id, layer.ref_count);
        } else {
            // 创建新层
    ...
    pub fn get_image_layers(&self, image_id: &str) -> Option<Vec<ImageLayer>> {
        ...
        let layers: Vec<ImageLayer> = layer_ids.iter()
    ...
    pub fn garbage_collect(&mut self, content_store: &ContentStore) -> Result<(usize, u64)> {
        ...
        for layer in &unreferenced {
            ...
            info!("Garbage collected layer: {} (freed {} bytes)", layer.id, layer.size);
        }
        
        self.save_index()?;
        
        info!("Garbage collection complete: deleted {} layers, freed {} bytes", 
...
impl LayerManager {
    ...
    /// 存储层内容
    pub fn store_layer(&mut self, data: &[u8], parent_id: Option<String>, media_type: &str) -> Result<String> {
        // 存储内容
        let hash = self.content_store.store_content(data)?;
        
        // 添加到层索引
        let size = data.len() as u64;
        self.layer_store.add_layer(hash.clone(), parent_id, size, media_type)?;
    ...
    pub fn has_layer(&self, layer_id: &str) -> bool {
        self.layer_store.get_layer(layer_id).is_some() && 
        self.content_store.has_content(layer_id)
    }
...
mod tests {
    ...
    fn test_layer_store() {
        ...
        // 关联到镜像
        store.associate_layers("image-1", vec!["sha256:abc123".to_string()]).unwrap();
    ...
    fn test_layer_manager() {
        ...
        let layer_id = manager.store_layer(data, None, "application/vnd.oci.image.layer.v1.tar+gzip").unwrap();
        
        // 验证层存在
        assert!(manager.has_layer(&layer_id));
        
        // 添加镜像层
        manager.add_image_layers("image-1", vec![layer_id.clone()]).unwrap();
