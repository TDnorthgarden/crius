...
impl StorageManager {
    ...
    /// 更新容器状态
    pub fn update_container_state(&mut self, container_id: &str, new_state: &str, exit_code: Option<i32>) -> Result<()> {
        // 获取旧状态
        let old_state: Option<String> = self.conn.query_row(
        ...
        if old_state.as_deref() != Some(new_state) {
            self.record_state_event("container", container_id, old_state.as_deref(), Some(new_state))?;
        }
    ...
    pub fn list_containers_by_pod(&self, pod_id: &str) -> Result<Vec<ContainerRecord>> {
        ...
        let records = stmt.query_map([pod_id], |row| {
            Ok(ContainerRecord {
                id: row.get(0)?,
                pod_id: row.get(1)?,
                state: row.get(2)?,
    ...
    pub fn delete_pod_sandbox(&mut self, pod_id: &str) -> Result<()> {
        // 先删除关联的容器
        self.conn.execute(
            "DELETE FROM containers WHERE pod_id = ?1",
            [pod_id],
        ).context("Failed to delete pod containers")?;

        // 再删除Pod
        self.conn.execute(
            "DELETE FROM pod_sandboxes WHERE id = ?1",
            [pod_id],
        ).context("Failed to delete pod sandbox")?;

        debug!("Pod sandbox {} and its containers deleted from database", pod_id);
        Ok(())
    }
    ...
    pub fn get_recent_events(&self, entity_type: &str, since: i64) -> Result<Vec<StateEvent>> {
        ...
        let events = stmt.query_map([entity_type, &since.to_string()], |row| {
            Ok(StateEvent {
                id: row.get(0)?,
    ...
    pub fn close(self) -> Result<()> {
        self.conn.close().map_err(|e| anyhow::anyhow!("Failed to close database: {:?}", e))?;
        Ok(())
    }
}
