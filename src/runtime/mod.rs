...
use crate::oci::spec::{Spec, Process, Root, Linux, Mount, User};
...
impl RuncRuntime {
    ...
    /// 执行runc命令并检查状态
    fn runc_exec(&self, args: &[&str]) -> Result<Output> {
        let output = self.run_command(args)?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            error!("runc command failed: {}", stderr);
            return Err(anyhow::anyhow!("runc command failed: {}", stderr));
        }
    ...
    fn create_spec(&self, config: &ContainerConfig, _container_id: &str) -> Result<Spec> {
        ...
        let mut all_mounts = default_mounts;
    ...
    /// 创建bundle目录结构
    fn create_bundle(&self, container_id: &str, _rootfs: &Path, spec: &Spec) -> Result<()> {
        ...
        // 创建rootfs目录（如果不存在）
        let bundle_rootfs = self.rootfs_path(container_id);
    ...
    fn get_runc_state(&self, container_id: &str) -> Result<Option<RuncState>> {
        let output = self.run_command(&["state", container_id])?;
...
impl ContainerRuntime for RuncRuntime {
    fn create_container(&self, config: &ContainerConfig) -> Result<String> {
        ...
        debug!("runc create output: {}", String::from_utf8_lossy(&output.stdout));
    ...
    fn start_container(&self, container_id: &str) -> Result<()> {
        ...
        if let Some(ref shim_manager) = self.shim_manager {
            ...
            info!("Container {} started via runc", container_id);
        }
    ...
    fn stop_container(&self, container_id: &str, timeout: Option<u32>) -> Result<()> {
        ...
        if let Ok(Some(state)) = self.get_runc_state(container_id) {
            if state.status != "stopped" {
                ...
                let _ = self.run_command(&["kill", container_id, "KILL"]);
            }
        }
    ...
    fn remove_container(&self, container_id: &str) -> Result<()> {
        ...
        // 删除容器
        let output = self.run_command(&["delete", container_id])?;
