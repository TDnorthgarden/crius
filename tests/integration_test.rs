...
fn check_runc_installed() -> bool {
    Command::new("which").arg("runc").output()
...
async fn test_container_lifecycle() {
    ...
    use crius::runtime::{ContainerRuntime, RuncRuntime, ContainerConfig, ContainerStatus};
    ...
    // 1. 创建容器
    let container_id = runtime.create_container(&config).expect("Failed to create container");
    println!("Container created: {}", container_id);

    // 验证状态为Created
    let status = runtime.container_status(&container_id).expect("Failed to get status");
    assert_eq!(status, ContainerStatus::Created, "Container should be in Created state");

    // 2. 启动容器
    runtime.start_container(&container_id).expect("Failed to start container");
    ...
    // 4. 停止容器（即使已经停止也不会出错）
    runtime.stop_container(&container_id, Some(5)).expect("Failed to stop container");
    println!("Container stopped");

    // 5. 删除容器
    runtime.remove_container(&container_id).expect("Failed to remove container");
    println!("Container removed");

    // 验证容器不存在
    let status = runtime.container_status(&container_id).expect("Failed to get status");
    assert_eq!(status, ContainerStatus::Unknown, "Container should be unknown after removal");
}
...
async fn test_stop_graceful_timeout() {
    ...
    let status = runtime.container_status(&container_id).expect("Failed to get status");
    assert_eq!(status, ContainerStatus::Running, "Container should be running");

    // 停止容器（使用3秒超时）
    let start = std::time::Instant::now();
    runtime.stop_container(&container_id, Some(3)).expect("Failed to stop container");
...
fn test_default_namespaces() {
    ...
    let ns_types: Vec<String> = namespaces.iter()
        .map(|n| n.ns_type.clone())
        .collect();
...
fn test_default_mounts() {
    ...
    // 检查是否有基本挂载点
    let destinations: Vec<String> = mounts.iter()
